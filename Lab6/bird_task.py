...
class Bird:
    ...
    def __repr__(self):
        if self.eggs_count == 1:
            return f'{self.name} has {self.eggs_count} egg'
        else:
            return f'{self.name} has {self.eggs_count} eggs'
    def lay_egg(self):
        self.eggs_count += 1
    def get_eggs(self):
        eggs = []
        for i in range(self.eggs_count):
            eggs.append(Egg())
        return eggs
