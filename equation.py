...
class Polynomial:
    def __init__(self, coeffs):
        if type(coeffs) == tuple:
            coeffs = list(coeffs)
        if coeffs and list(filter(lambda a: a!=0, coeffs)):
            filtered_coeffs = copy.deepcopy(coeffs)
            while filtered_coeffs[0] == 0:
                filtered_coeffs.pop(0)
            self.coeffs = filtered_coeffs
        else:
            self.coeffs = [0]
    def __repr__(self):
        return f'Polynomial(coeffs=[{", ".join(map(str, self.coeffs))}])'
    def degree(self):
        return len(self.coeffs)-1
    def coeff(self, power):
        index = len(self.coeffs) - 1 - power
        return self.coeffs[index]
    def evalAt(self, x):
        sum = 0
        for index in range(len(self.coeffs)):
            sum += self.coeffs[index]*x**(len(self.coeffs)-1-index)
        return sum
    def __eq__(self, other):
        if not (isinstance(other, Polynomial)) and type(other) != int:
            return False
        elif type(other) == int and (len(self.coeffs) == 1 and ''.join(map(str, self.coeffs)) == str(other)):
            return True
        elif isinstance(other, Polynomial) and (self.coeffs == other.coeffs or self.coeffs == [0] and other.coeffs == []):
            return True
        return False
